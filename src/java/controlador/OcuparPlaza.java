...
public class OcuparPlaza extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		if(sesion!=null){
                    ...
                    if(p.getTipoVehiculo().equals(v.getTipoVehiculo())==false){

                        request.setAttribute("mensajeErrorVehiculoNoValido","¡El tipo de vehículo elegido no es compatible con la plaza seleccionada!");
			RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/reservaPlaza.jsp?error");
			dispatcher.forward(request, response);
                        
                    }else if(p.getTipoVehiculo().equals("coche") && v.getTipoVehiculo().equals("coche") && p.isElectrico()==true && v.isElectrico()==false){
                    
                        request.setAttribute("mensajeErrorVehiculoNoValido","¡La plaza seleccionada sólo puede ser ocupada por un coche de tipo eléctrico!");
			RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/reservaPlaza.jsp?error");
			dispatcher.forward(request, response);
